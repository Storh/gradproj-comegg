...
class TopicService extends Service {
  async getListById(user_id, reqData) {
    ...
    const likeType = 2;
    const contentType = this.app.config.contentTypeIdByName.TOPIC;
    ...
    const sqlstr = `SELECT
                        ...
                        FROM ${this.app.config.dbprefix}topic_regist a
  ...
  async registAdd(user_id, reqData) {
    ...
    const regist_log = await this.app.mysql.insert(this.app.config.dbprefix + 'topic_regist', {
      user_id,
